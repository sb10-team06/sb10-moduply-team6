...
CREATE TABLE notifications (
    ...
    -- 알림 유형 값 검증
                               CONSTRAINT chk_notifications_type CHECK (type IN ('ROLE_CHANGED', 'PLAYLIST_SUBSCRIBED', 'CONTENT_ADDED', 'FOLLOWED', 'FOLLOW_ACTIVITY', 'DM_RECEIVED')),
    -- 알림 수준 값 검증
...
CREATE TABLE content_tags (
    ...
    -- 동일 콘텐츠에 같은 태그 중복 등록 방지
                              CONSTRAINT uk_content_tags_content_tag
...
CREATE TABLE conversations (
    ...
    -- 주의: A-B와 B-A는 다른 값이므로 저장 순서 고정 필요
                               CONSTRAINT uk_conversations_user_pair
